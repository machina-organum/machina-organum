...
def options_and_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Machina-Organum thinks for you about the meanings.",
        epilog="Example:  machina-organum input_text.txt > output_text.txt"
...
def run():
    """
    $ text | machina-organum                        # Accepts text from the pipe
    $ echo "...<text>..." | machina-organum         #

    $ machina-organum multilogue.txt new_turn.txt    # ...or files.
    """
    ...
    try:
        ...
        syslog.openlog(
            ident="machina-organum",
            logoption=syslog.LOG_NDELAY,
            facility=syslog.LOG_USER
        )
        # Signal (single line less than 4096 only!)
        syslog.syslog(syslog.LOG_INFO, f"machina-organum: {utterance}.")
        syslog.closelog()
